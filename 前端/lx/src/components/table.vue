    ...
    <el-table
      ...
      <el-table-column label="操作" align="center">
        <template slot-scope="scope">
          ...
          <el-button
            v-if="modal=='commodity'"
            size="mini"
            :type="scope.row.status=='上架'?'danger':'primary'"
  ...
  export default {
    ...
    methods: {
      ...
      count() {
        let param = JSON.parse(JSON.stringify(this.searchData))
        if (this.modal == 'manager') {
          this.$post('/manager/count', param, res => {
            this.total = res.data

          })
        }
        if (this.modal == 'commodity') {
          this.$post('/commodity/count', param, res => {
            this.total = res.data

          })
        }
        if (this.modal == 'user') {
          this.$post('/user/count', param, res => {
            this.total = res.data
